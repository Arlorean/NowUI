...
// ---------------------------------------------------------------------------------------------------- boot report
//
// index.html paints "Starting NowUI..." over the canvas and this owns the rest of that element's life: it becomes
// the error if boot fails, and it is removed on the first frame that actually draws.
//
// Why it exists: everything below this point is one long top-level await, so any failure - a bundle file the server
// did not serve, a wasm the browser refused on an integrity mismatch, a syntax error in the author's ?app= file -
// leaves a black window and a single console line. The difference between "it didn't work" and a bug report is
// about fifteen lines, and these are them.
const bootNode = document.getElementById('nowui-boot');
let bootCleared = false;

function bootProblem(what, detail) {
    if (!bootNode || bootCleared) return;
    bootNode.classList.add('nowui-boot-failed');
    bootNode.textContent = 'NowUI could not start.\n\n' + what + '\n\n' + detail +
        '\n\nThe page is served from ' + location.origin + '. Open the browser console for the full trace.';
}
...
function bootDone() {
    ...
    if (bootWatchdog !== null) { clearTimeout(bootWatchdog); bootWatchdog = null; }
}

// Everything the managed side reported at error level before the first frame. Program.cs's RunAsync catches its
// own start-up exceptions and reports them through host.log rather than letting them reject anything JavaScript
// can see, so this is the only place the browser half can learn that a `?app=` module failed to import or that a
// shader would not compile. Kept rather than shown on arrival because not every level-2 line is fatal - the
// fetch bridge logs one and carries on - and an error banner that a working page paints over half a second later
// is worse than none.
const bootErrors = [];
const BOOT_ERROR_SEPARATOR = '\n\n';
let bootWatchdog = null;

function bootReportedError(message) {
    if (bootCleared) return;
    bootErrors.push(String(message));

    // Program.cs's own words for "this is over". Shown at once, because nothing is coming after it.
    if (/start-up failed/.test(message)) {
        bootProblem('NowUI reported a start-up failure.', bootErrors.join(BOOT_ERROR_SEPARATOR));
        return;
    }
    ...
    if (bootWatchdog === null) {
        bootWatchdog = setTimeout(() => {
            bootWatchdog = null;
            if (bootCleared) return;
            bootProblem('NowUI has not drawn a frame, and reported this on the way up.',
                bootErrors.join(BOOT_ERROR_SEPARATOR));
        }, 12000);
    }
}

// A module whose top-level await rejects surfaces through one of these two, depending on the browser; both are
// registered rather than guessing which.
window.addEventListener('error', (event) => {
    bootProblem('A script failed while the page was starting.',
        String((event && event.error && event.error.stack) || (event && event.message) || event));
});
window.addEventListener('unhandledrejection', (event) => {
    const reason = event && event.reason;
    bootProblem('Start-up failed.', String((reason && reason.stack) || reason));
});

// ------------------------------------------------------------------------------------------------- report panel
//
// bridge.js draws a diagnostic <pre> across the bottom 45% of the page. That is right when the report IS the
// picture (?bridge=), and wrong when the picture is an author's application and the bottom of it is the part
// being looked at - so the default is inverted HERE, in the host page, by a stylesheet rule rather than by
// touching the bridge: ON for everything except ?app=, and ?report=1 forces it back on either way.
//
// A rule rather than a removal on purpose: the element is the oracle several headless drivers read, and
// window.__nowuiBridgeReport is untouched, so hiding it changes what a person sees and nothing that a driver
// reads. ?report=0 already suppressed it entirely and still does.
(() => {
    const search = new URLSearchParams(location.search);
    if (search.get('report') === '1') return;
    if (!search.has('app')) return;

    const style = document.createElement('style');
    style.textContent = '#nowui-bridge-report { display: none !important; }';
    ...
    host: {
        ...
        log: (level, message) => {
            if (level >= 2) {
                ...
                // The one route a managed start-up failure has into the page. See bootReportedError.
                bootReportedError(message);
            }
...
// ------------------------------------------------------------------------------------------------------- ?watch=1
//
// The authoring loop, for the Editor's Web Preview: with ?watch=1 and ?app=NAME the page asks the local server for
// the app file's last-write time twice a second and reloads when it moves, so "save, alt-tab, F5" becomes "save".
//
// A poll of one endpoint rather than a WebSocket or a FileSystemWatcher, deliberately: the browser already has an
// F5 key and this is meant to add as little machinery as possible. The endpoint is served only by the Unity
// Editor's preview server (NowWebPreviewServer.HandleMtime); anywhere else the fetch 404s and this turns itself
// off after the first answer, so the flag is inert on the repo's own dev server.
(() => {
    const search = new URLSearchParams(location.search);
    const app = search.get('app');
    if (search.get('watch') !== '1' || !app) return;

    const endpoint = './__nowui/mtime?app=' + encodeURIComponent(app);
    let baseline = null;
    ...
    const poll = async () => {
        ...
        try {
            ...
            if (!response.ok) return;              // not the Editor's server: stop asking.
            text = (await response.text()).trim();
        } catch (e) {
            return;
        }
        if (text === '0' || text === '') { setTimeout(poll, 500); return; }
        if (baseline === null) baseline = text;
        else if (text !== baseline) { location.reload(); return; }
        setTimeout(poll, 500);
    };
...
try {
    ...
    bootProblem('Main() threw while bringing NowUI up.', String((e && e.stack) || e));
    throw e;
}
