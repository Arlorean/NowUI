// A managed PNG codec, for the images that never pass through the browser's decoder.
//
// WHY THIS EXISTS. The browser host's decoder answers from a cache the fetch provider fills; bytes that never went
// through fetch() - a local asset, a byte[] built in code, an EncodeToPNG round trip - have no cache entry and
// would otherwise have no decoder at all. PNG is the format worth doing by hand: its hard part is DEFLATE, and
// System.IO.Compression already has that, so what is left is a chunk walk, five row filters and a pixel expansion.
// JPEG is deliberately not here (see WebImageDecoder's header).
//
// SCOPE, stated exactly, because a decoder that quietly mangles an input it does not really support is worse than
// one that refuses it:
//
//   colour type 0 (greyscale)        bit depths 1, 2, 4, 8, 16      tRNS honoured
//   colour type 2 (truecolour)       bit depths 8, 16               tRNS honoured
//   colour type 3 (palette)          bit depths 1, 2, 4, 8          tRNS honoured (per-entry alpha)
//   colour type 4 (greyscale+alpha)  bit depths 8, 16
//   colour type 6 (truecolour+alpha) bit depths 8, 16
//   interlace 0 (none) only          Adam7 is REFUSED by name, not approximated
//
// 16-bit samples are reduced to 8 by taking the high byte, which is what every 8-bit consumer of a 16-bit PNG does
// and what NowUI's RGBA32 textures can hold. Ancillary chunks (gAMA, iCCP, sRGB, cHRM, tEXt, ...) are SKIPPED, not
// applied: NowRuntime.colorSpace is Gamma and the whole browser colour path is written for untouched bytes
// (M2-ShaderPort.md §6), so applying a gamma chunk here would be the one place in the pipeline that second-guesses
// the file. CRCs are not verified; a corrupt chunk fails the inflate or the size check instead, which is the same
// refusal one step later.
//
// Output is tightly packed RGBA32 rows, BOTTOM-UP, which is INowImageDecoder's contract and Texture2D's own store.
using System;
using System.IO;
...
namespace NowUI.Web
{
    ...
    internal static class NowWebPng
    {
                    ...
                    case k_Ihdr:
                        ...
                        if (chunk[12] != 0)
                        {
                            error = "The PNG is Adam7 interlaced, which this managed decoder does not implement. " +
                                    "An interlaced PNG fetched over the network still decodes, through the " +
                                    "browser's own codec.";
                            return false;
                        }
